...
def savefig(path, **args):
    logging.info("[plots.savefig] Saving plot to %s" % path)
    
    reset = args.pop("reset", True)
    
    if path in savefig.paths:
        logging.warn("[plots.savefig] Overwritting %s" % path)
...
def densities_plot(label2values, **kwargs):
    ...
    for i, (label, values) in enumerate(sorted(label2values.items())):
        ...
        try:    
            ...
            print("WARNING: [densities_plot: sns.kdeplot] - %s" % e) 
            sns.kdeplot(np.array(values), label=label, ax=axis, color=c)
