...
#    argv = sys.argv[0]
def run(argv):
    parser = argparse.ArgumentParser(description="Samples from non-homogenous Poisson temporal point processes with known maximum.")
    ...
    if args.verbose:
        #print("[thinning] configuring logging serice.")
        fmt = '[%(process)4d][%(asctime)s][%(levelname)-5s][%(module)s:%(lineno)d/%(funcName)s] %(message)s'        
        logging.basicConfig(level=logging.INFO, format=fmt) 
        logging.getLogger().addHandler(logging.StreamHandler())
    logging.info("[thinning] Parsed args: %s" % args)    
    logging.info("[thinning] Unparsed args: %s" % process_args)    
    ...
    logging.info("[thinning] preparing a process factory from the description file %s" % args.description)
    ...
    logging.info("[thinning] constructing processes")
    processes = list(process_factory.yield_processes()) 
    
    logging.info("[thinning] generating samples for %i processes" % len(processes))
    for i, p in enumerate(processes):
        if len(processes)<=20 or i%(len(processes)//20)==0: logging.debug("[thinning] generating samples: %i/%i" % (i, len(processes)))
        list(yield_point_process(p.intensity, p.update, p.max_intensity(), p.max_time()))
                                                    
    ####################################################################  
          
    logging.info("[thinning] merging samples")
    ...
    if args.loglikelihood is not None:
        logging.info("[thinning] saving likelihoods to %s" % args.loglikelihood)
        df = pd.DataFrame()
        df["ll"] = pd.Series(p.ll() for p in processes)
        df.to_csv(args.loglikelihood, sep="\t", index=False)
        logging.info("[thinning] ll=%f" % df["ll"].sum())
